...
for hyp=1:hyp_no
    for j=1:M
        ...
        p(hyp,j).xL=obs{hyp}(3,1)+(-5+r*10); %randomize bw -5 to 5 m around prior belief
        p(hyp,j).yL=obs{hyp}(4,1)+(-5+r2*10); %randomize bw -5 to 5 m around prior belief
        p(hyp,j).xR=-0.25+p(hyp,j).xL+r*0.5; %randomize bw -5 to 5 m around prior belief of left foot
        p(hyp,j).yR=-0.25+p(hyp,j).yL+r2*0.5; %randomize bw -5 to 5 m around prior belief of left foot
...
for i=1:total_obs %we will only simulate until the observations are exhausted
    ...
    if (mod(i,3)== 0) && (size(map,2) > 0)
        ...
        w=update_pfilter_multihyp(hyp_cols,p,map,USE_IMU_TH);
        resamp=1;
    end
...
for hyp=1:size(obs,1)%size(map,2)
    ...
    plot(xhat_ser{hyp}(:),yhat_ser{hyp}(:),'color',[rgb{hyp}.r rgb{hyp}.g rgb{hyp}.b],'LineWidth',1);

    obs{hyp,1}(3,:)=obs{hyp,1}(3,:);
    obs{hyp,1}(4,:)=obs{hyp,1}(4,:);    
    obs{hyp,1}(3,:)=smooth(obs{hyp,1}(3,:));
    obs{hyp,1}(4,:)=smooth(obs{hyp,1}(4,:));    
    obs{hyp,1}(3,:)=smooth(obs{hyp,1}(3,:));
    obs{hyp,1}(4,:)=smooth(obs{hyp,1}(4,:));
    
    plot(obs{hyp,1}(3,:),obs{hyp,1}(4,:),':','color',[rgb2{hyp}.r rgb2{hyp}.g rgb2{hyp}.b],'LineWidth',1);%'markersize',5);    
